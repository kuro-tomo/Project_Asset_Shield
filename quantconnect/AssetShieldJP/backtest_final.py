...
class AssetShieldBacktest(QCAlgorithm):
    ...
    Uses ticker format: 7203, 6758, 8306 (matching upload)
    """

    # Match the tickers from your DataUploaderFinal
    ...
    def Initialize(self):
        ...
        if len(self.symbols) == 0:
            raise Exception("No symbols loaded - run DataUploaderFinal first")
        ...
        for ticker, symbol in self.symbols.items():
            if slice.ContainsKey(symbol):
                ...
                if bar and bar.Value > 0:
                    ...
                    self.data_count += 1
                    self.Debug(f"[{self.Time.date()}] {ticker}: ¥{bar.Value:,.0f}")
    ...
    def Rebalance(self):
        ...
        self.Debug(f"[{self.Time.date()}] REBALANCE | Equity: ¥{equity:,.0f} | Active: {active}")

        # Equal weight allocation
        weight = 1.0 / len(self.TICKERS) if self.TICKERS else 0
        for ticker, symbol in self.symbols.items():
            if self.prices[ticker] > 0:
                self.SetHoldings(symbol, weight * 0.95)  # 95% to leave cash buffer

    def OnEndOfAlgorithm(self):
        equity = self.Portfolio.TotalPortfolioValue
        ret = (equity / 10_000_000) - 1
        ...
        if self.data_count > 0:
            self.Debug("✓ SUCCESS - Data pipeline verified!")
        else:
