#!/usr/bin/env python3
"""V5.5 Pure Momentum - Full Universe
NO fundamental filters. Pure 12-1 Momentum. Top 20 by momentum only.
"""
...
def run_segment(segment_id, start_date, end_date, output_path):
    ...
    logger.info(f"V5.5 PURE MOMENTUM: {start_date} to {end_date}")
    ...
    for i, dt in enumerate(trading_days):
        ...
        for c in positions:
            if c in prices:
                ...
                if len(avail) > 0:
                    pos_val += positions[c] * avail.iloc[-1]['close']
        equity = cash + pos_val
        if equity > hwm:
            hwm = equity
        dd = (hwm - equity) / hwm
        ...
        if (last_rebal is None or (dt - last_rebal).days >= 20) and max_exp > 0:
            ...
            for code in prices:
                ...
                # ADT filter: 50M+ JPY (liquid stocks only)
                if adt < 50_000_000:
                    continue
            ...
            for code, mom, adt in candidates[:50]:
            ...
            last_rebal = dt
            if i % 200 == 0:
                top_mom = candidates[0][1] if candidates else 0
                logger.info(f"{dt}: {len(candidates)} liquid, top_mom={top_mom:.2%}, {regime}")
